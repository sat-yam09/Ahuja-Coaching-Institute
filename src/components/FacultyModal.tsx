...
export const FacultyModal: React.FC<FacultyModalProps> = ({ faculty, onClose, onInquire }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
      <div className="relative w-full max-w-2xl bg-[#FAF6EE] rounded-2xl shadow-2xl border border-[#E5DCCB] overflow-hidden my-auto max-h-[90vh] flex flex-col animate-scale-up">
        {/* Top Header */}
        <div className="bg-[#5C1315] text-white p-6 relative flex items-start justify-between flex-shrink-0">
          <div className="flex items-center gap-4">
            <img
              src={faculty.avatarUrl}
              alt={faculty.name}
              className="w-20 h-20 rounded-full object-cover border-2 border-amber-400 shadow-md flex-shrink-0"
            />
            <div>
              ...
              <h3 className="text-2xl font-bold font-serif text-white">{faculty.name}</h3>
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-gray-800">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white p-3 rounded-xl border border-gray-200 text-center">
              <Clock className="w-5 h-5 text-[#5C1315] mx-auto mb-1" />
              <div className="text-xs text-gray-500">Experience</div>
              <div className="font-bold text-[#5C1315] text-sm">{faculty.experience}</div>
            </div>
            <div className="bg-white p-3 rounded-xl border border-gray-200 text-center">
              <GraduationCap className="w-5 h-5 text-[#5C1315] mx-auto mb-1" />
          ...
          {/* Biography */}
          <div>
            <h4 className="text-xs font-bold text-[#5C1315] uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <BookOpen className="w-4 h-4" /> Teaching Philosophy & Background
            </h4>
            <p className="text-sm leading-relaxed text-gray-700 bg-white p-4 rounded-xl border border-gray-200">
              {faculty.bio}
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {faculty.achievements.map((ach, i) => (
                <li key={i} className="flex items-start gap-2 text-xs text-gray-700 bg-white p-2.5 rounded-lg border border-gray-200">
        ...
        <div className="p-4 bg-white border-t border-gray-200 flex items-center justify-between flex-shrink-0">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-[#5C1315] hover:bg-[#430d0f] text-white font-bold rounded-xl text-xs transition shadow-xs"
          >
